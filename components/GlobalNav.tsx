...
export const GlobalNav: React.FC<GlobalNavProps> = ({ activePage, onNavigate }) => {
  ...
  return (
    <nav className={`${isCollapsed ? 'w-20' : 'w-64'} bg-white flex-shrink-0 border-r border-slate-200 flex flex-col h-full transition-all duration-300 ease-in-out z-50`}>
      {/* Sidebar Header / Logo Area */}
      <div className={`h-16 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between px-6'} border-b border-slate-200 flex-shrink-0 transition-all duration-300`}>
        <div 
        ...
        >
            ...
            {!isCollapsed && (
                <span className="text-lg font-bold text-slate-800 tracking-tight whitespace-nowrap animate-in fade-in duration-300">Nova Vacation</span>
            )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pb-10 pt-2 overflow-x-hidden">
        {SECTIONS.map((section, idx) => (
            ...
            {/* Section Title */}
            {section.title && (
                <h3 className={`text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2 mt-4 transition-opacity duration-300 ${isCollapsed ? 'hidden' : 'px-2 opacity-100'}`}>
                  {section.title}
                </h3>
            )}
            {isCollapsed && section.title && <div className="h-px bg-slate-100 my-4 mx-2"></div>}
            ...
            <ul className="space-y-0.5">
                ...
                <li key={item.id}>
                    ...
                    >
                    <div className={`w-5 h-5 flex-shrink-0 ${activePage === item.id ? 'text-purple-600' : 'text-slate-400'}`}>
                        <item.icon />
                    </div>
