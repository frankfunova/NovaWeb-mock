...
const OtaBadge: React.FC<{ ota: string }> = ({ ota }) => {
    ...
    return (
        <span className={`px-2 py-0.5 rounded border text-[9px] font-bold uppercase tracking-wide ${styles}`}>
            {ota}
        </span>
    );
};
...
export const ReviewDetail: React.FC<ReviewDetailProps> = ({ review: initialReview }) => {
  ...
  const handlePostReply = () => {
      ...
      setReview(updatedReview as Review); // Cast because types might differ slightly in mock vs real
      setIsReplying(false);
      setReplyText('');
  };
  ...
  const generateMockReviews = (startId: number, count: number) => {
      return Array.from({ length: count }).map((_, i) => {
          ...
          reviewDateObj.setDate(today.getDate() - (id * 14) - 30); // Spread out dates
  ...
  const handleTogglePrevReviews = () => {
      if (!isPrevReviewsOpen && prevReviews.length === 0) {
          ...
          // Simulate Fetch of first 5
          setTimeout(() => {
               const newReviews = generateMockReviews(0, 5);
               setPrevReviews(newReviews);
               setHistoricSummary({ avg: 4.7, count: 42 }); // Mock summary
  ...
  return (
    <div className="flex flex-col h-full bg-white overflow-y-auto custom-scrollbar">
        ...
        {/* Main Content */}
        <div className="flex-1 px-6 pt-0 pb-12">
            
            {/* 1. Interactive Rating Summary & Details */}
            <div 
                className="mb-6 mt-4 rounded-xl p-4 transition-all hover:bg-slate-50/50 hover:shadow-sm border border-transparent hover:border-slate-100 cursor-default group -mx-4"
            ...
            >
                <div className="flex items-center justify-between">
                     ...
                     <div className="text-right">
                         <div className="text-xs font-bold text-slate-700 uppercase tracking-wide">REVIEWED ON</div>
                         <div className="text-xs text-slate-400 mt-0.5">{new Date(review.reviewDate).toLocaleDateString()}</div>
                     </div>
                </div>
            ...
            {/* 3. Assigned Team Section (Moved Above Previous Reviews) */}
            <div className="mb-8">
                <div className="flex items-center gap-2 mb-3">
                    <Icons.Users className="w-4 h-4 text-slate-400" />
                    <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Assigned Team</h3>
                </div>
            ...
            {/* 4. AI Insight (Moved Above Previous Reviews) */}
            ...
            {/* 5. Previous Reviews Section (Moved to Bottom) */}
